...
@Roles('LANDLORD', 'ADMIN', 'STAFF') // <-- FIX: Allow Staff to access the controller (Needed for dashboard loading)
export class LandlordsController {
  constructor(private readonly landlordsService: LandlordsService) {}

  @Post('profile')
  @Roles('LANDLORD', 'ADMIN') // <-- SECURE: Only Landlords/Admins can create a company
  async createProfile(
    @Request() req: any, 
  ...
  @Put('profile')
  @Roles('LANDLORD', 'ADMIN') // <-- SECURE: Staff cannot edit the Landlord's master profile or API keys!
  async updateProfile(
    @Request() req: any,
    @Body() updateDto: any 
  ) {
