...
export class PdfService {
    async generatePaymentReceipt(data: {
        ...
        return new Promise((resolve, reject) => {
            ...
            const primaryColor = '#0f3e46'; // Deep Teal
            const accentColor = '#1f8898';  // Light Teal
            const lightGray = '#f3f4f6';

            // --- 1. Watermark ---
            doc.save();
            // Set rotation origin to the exact center of the A4 page
            doc.rotate(-35, { origin: [doc.page.width / 2, doc.page.height / 2] });
            
            // Adjust opacity, scale font, and center align within the full page width
            doc.fillColor('#9ca3af').fillOpacity(0.15).fontSize(130).font('Helvetica-Bold')
            ...
            doc.font('Helvetica-Bold').fontSize(14).text(data.tenantName, 50, 155);
            doc.font('Helvetica').fontSize(11).fillColor('#4b5563').text(`${data.propertyName} - Unit ${data.unitNumber}`, 50, 172);
            ...
            doc.fillColor('#4b5563').font('Helvetica-Bold').text('Powered by Mogitech Global Ltd', 0, bottom + 26, { align: 'center', width: doc.page.width, lineBreak: false });
