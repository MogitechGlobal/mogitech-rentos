...
import { 
  ...
  Sparkles, ShieldCheck, ChevronDown, Check
} from 'lucide-react';
...
export default function UnitDetailsPage() {
  ...
  // Accordion state for the edit modal
  const [expandedCategory, setExpandedCategory] = useState<'internal' | 'external' | null>('internal');
  ...
  const handleEditUnit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Update Base Unit Details (Unit Number, Rent)
      const res1 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/units/${unitId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        credentials: 'include',
        body: JSON.stringify({ unit_number: formData.unit_number, rent_amount: Number(formData.rent_amount) })
      });
      if (!res1.ok) throw new Error('Failed to update basic unit details');

      // 2. Update Specifications and Amenities via the Listing Endpoint
      const res2 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/units/${unitId}/listing`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        credentials: 'include',
        body: JSON.stringify({ 
      ...
      if (!res2.ok) throw new Error('Failed to update unit specifications');
  ...
  // Separate unit amenities for display
  const savedInternalAmenities = unit.amenities?.filter((a: string) => AMENITIES_CATEGORIES.internal.includes(a)) || [];
  const savedExternalAmenities = unit.amenities?.filter((a: string) => AMENITIES_CATEGORIES.external.includes(a)) || [];
    ...
    <div className="min-h-screen bg-[#f8fafb] pb-12 font-sans selection:bg-[#1f8898]/30 overflow-x-hidden">
            ...
            <div className="space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-4 sm:gap-6">
                        ...
                        {/* --- NEW: FEATURES & SPECIFICATIONS CARD --- */}
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                            <div className="p-4 sm:p-6 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                                <div>
                                    <h2 className="text-base sm:text-lg font-black text-gray-900 flex items-center gap-2">
                                        <Sparkles className="w-4 h-4 text-[#1f8898]" /> Features & Specifications
                                    </h2>
                                    <p className="text-[10px] sm:text-xs text-gray-500 font-medium mt-0.5">Amenities mapped to marketplace filters</p>
                                </div>
                                <button onClick={() => openModal('EDIT_UNIT')} className="text-xs font-bold text-[#1f8898] hover:underline flex items-center gap-1">
                                    <Edit className="w-3 h-3"/> Edit Specs
                                </button>
                            </div>
                            <div className="p-4 sm:p-6 flex flex-col gap-6">
                                {savedInternalAmenities.length === 0 && savedExternalAmenities.length === 0 ? (
                                    <div className="text-center py-6 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                                        <ShieldCheck className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                                        <p className="text-sm font-bold text-gray-500">No amenities configured yet.</p>
                                        <button onClick={() => openModal('EDIT_UNIT')} className="mt-2 text-xs font-bold text-[#1f8898] hover:underline">Configure Amenities &rarr;</button>
                                    </div>
                                ) : (
                                    <>
                                        {savedInternalAmenities.length > 0 && (
                                            <div>
                                                <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">Internal Features</h3>
                                                <div className="flex flex-wrap gap-2">
                                                    {savedInternalAmenities.map((amenity: string) => (
                                                        <span key={amenity} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#ebf3f5]/50 border border-[#1f8898]/20 text-[#1f8898] rounded-lg text-xs font-bold shadow-sm">
                                        ...
                                        {savedExternalAmenities.length > 0 && (
                                            <div>
                                                <h3 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">External Features</h3>
                                                <div className="flex flex-wrap gap-2">
                                                    {savedExternalAmenities.map((amenity: string) => (
                                                        <span key={amenity} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-200 text-gray-700 rounded-lg text-xs font-bold shadow-sm">
                                                            <CheckCircle2 className="w-3.5 h-3.5 text-gray-400" /> {amenity}
                                                        </span>
                                                    ))}
                                                </div>
                                            </div>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in">
          <div className={`bg-white rounded-[2rem] shadow-2xl w-full ${activeModal === 'EDIT_UNIT' ? 'max-w-2xl' : 'max-w-md'} max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95`}>
            ...
            <div className="p-5 sm:p-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              {/* COMPREHENSIVE EDIT UNIT FORM */}
              {activeModal === 'EDIT_UNIT' && (
                <form id="modal-form" onSubmit={handleEditUnit} className="space-y-6">
                    {/* Basic Info */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input required type="text" value={formData.unit_number} onChange={e => setFormData({...formData, unit_number: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-bold outline-none focus:bg-white focus:border-[#1f8898]" />
                        </div>
                        <div>
                            ...
                            <input required type="number" value={formData.rent_amount} onChange={e => setFormData({...formData, rent_amount: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-bold outline-none focus:bg-white focus:border-[#1f8898]" />
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <h3 className="text-xs font-black uppercase tracking-widest text-gray-900">Specifications</h3>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            <div className="col-span-2 sm:col-span-1">
                                <label className="block text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1.5 sm:mb-2 ml-1">Unit Type</label>
                                <select value={formData.unit_type} onChange={e => setFormData({...formData, unit_type: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 sm:py-3 text-xs sm:text-sm font-bold outline-none focus:bg-white focus:border-[#1f8898]">
                            ...
                            <div>
                                <label className="block text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1.5 sm:mb-2 ml-1">Bedrooms</label>
                                <input type="number" value={formData.bedrooms} onChange={e => setFormData({...formData, bedrooms: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 sm:py-3 text-sm font-bold outline-none focus:bg-white focus:border-[#1f8898]" placeholder="e.g. 2" />
                            </div>
                            <div>
                                <label className="block text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1.5 sm:mb-2 ml-1">Bathrooms</label>
                                <input type="number" step="0.5" value={formData.bathrooms} onChange={e => setFormData({...formData, bathrooms: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 sm:py-3 text-sm font-bold outline-none focus:bg-white focus:border-[#1f8898]" placeholder="e.g. 1.5" />
                            </div>
                            <div className="col-span-2 sm:col-span-1">
                                <label className="block text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1.5 sm:mb-2 ml-1">Size (SQM)</label>
                                <input type="number" value={formData.size_sqm} onChange={e => setFormData({...formData, size_sqm: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 sm:py-3 text-sm font-bold outline-none focus:bg-white focus:border-[#1f8898]" placeholder="e.g. 85" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <h3 className="text-xs font-black uppercase tracking-widest text-gray-900">Amenities</h3>
                        <p className="text-xs text-gray-500 font-medium">Select features to map directly to marketplace filters.</p>
                        ...
                        <div className="border border-gray-200 rounded-2xl overflow-hidden">
                            ...
                            <div className="border-b border-gray-200">
                                <button 
                                    type="button" 
                                    onClick={() => setExpandedCategory(expandedCategory === 'internal' ? null : 'internal')}
                                    className="w-full flex items-center justify-between p-4 bg-gray-50/50 hover:bg-gray-50 transition-colors"
                                >
                                    <span className="font-bold text-sm text-gray-900">Internal Features</span>
                                    <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedCategory === 'internal' ? 'rotate-180' : ''}`} />
                                </button>
                                {expandedCategory === 'internal' && (
                                    <div className="p-4 bg-white grid grid-cols-1 sm:grid-cols-2 gap-3 border-t border-gray-100">
                                        {AMENITIES_CATEGORIES.internal.map((amenity: string) => {
                                            ...
                                            return (
                                                <button 
                                                    ...
                                                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                                                        isSelected ? 'bg-[#ebf3f5] border-[#1f8898]/30' : 'bg-white border-gray-200 hover:border-gray-300'
                                                    }`}
                                                >
                                                    <div className={`w-4 h-4 rounded shadow-sm border flex items-center justify-center shrink-0 ${isSelected ? 'bg-[#1f8898] border-[#1f8898]' : 'bg-gray-50 border-gray-300'}`}>
                                                        {isSelected && <Check className="w-3 h-3 text-white" />}
                                                    </div>
                                                    <span className={`text-xs font-bold ${isSelected ? 'text-[#1f8898]' : 'text-gray-600'}`}>{amenity}</span>
                                                </button>
                                            );
                                        })}
                                    </div>
                            ...
                            <div>
                                <button 
                                    type="button" 
                                    onClick={() => setExpandedCategory(expandedCategory === 'external' ? null : 'external')}
                                    className="w-full flex items-center justify-between p-4 bg-gray-50/50 hover:bg-gray-50 transition-colors"
                                >
                                    <span className="font-bold text-sm text-gray-900">External Features</span>
                                    <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedCategory === 'external' ? 'rotate-180' : ''}`} />
                                </button>
                                {expandedCategory === 'external' && (
                                    <div className="p-4 bg-white grid grid-cols-1 sm:grid-cols-2 gap-3 border-t border-gray-100">
                                        {AMENITIES_CATEGORIES.external.map((amenity: string) => {
                                            const isSelected = formData.amenities?.includes(amenity);
                                            return (
                                                <button 
                                                    key={amenity} type="button" 
            ...
            <div className="p-5 sm:p-6 border-t border-gray-100 bg-white flex justify-end gap-2 sm:gap-3 shrink-0">
